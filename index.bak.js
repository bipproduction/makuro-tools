...
if (!db.get(DB_KEY.config)) {
    db.set(DB_KEY.config, [])
}
...
async function checkVersion() {
    ...
    console.log("check update please wait ...")
    const ver = execSync('npm show makuro-tools version').toString().trim()
    const localVer = db.get(DB_KEY.version)
    if (!localVer || ver !== localVer) {
        console.log("new version available ...".cyan)
        console.log("Please wait ...")
        await new Promise((resolve) => {
            execSync(`npm update -g makuro-tools`)
            db.set(DB_KEY.version, ver)
            resolve()
        })
    }
}
...
async function funSetConfig() {
    ...
    db.push(DB_KEY.config, dataHasil)
...
async function funRemoveServer() {
    /**
             * @type {SERVER_CONFIG[]}
             */
    const lsConfig = db.get(DB_KEY.config)
